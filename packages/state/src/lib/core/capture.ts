...
const tldrawStateGlobalKey = Symbol.for('__@digitalsamba/state__')
const tldrawStateGlobal = globalThis as { [tldrawStateGlobalKey]?: true }

if (tldrawStateGlobal[tldrawStateGlobalKey]) {
	console.error(
		'Multiple versions of @digitalsamba/state detected. This will cause unexpected behavior. Please add "resolutions" (yarn/pnpm) or "overrides" (npm) in your package.json to ensure only one version of @digitalsamba/state is loaded.'
	)
} else {
